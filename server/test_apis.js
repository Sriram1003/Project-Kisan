...
async function testAPIs() {
  ...
  try {
    const wRes = await axios.get(`https://api.openweathermap.org/data/2.5/weather?q=Karimnagar,IN&appid=${process.env.VITE_OPENWEATHER_API_KEY}&units=metric`);
  ...
  console.log("\nTesting Mandi API...");
  try {
    const mRes = await axios.get(`https://api.data.gov.in/resource/35985678-0d79-46b4-9ed6-6f13308a1d24?api-key=${process.env.VITE_MANDI_API_KEY}&format=json&limit=1`);
  ...
  try {
    ...
    const pRes = await axios.post('https://api.plant.id/v2/identify', { images: [fakeImage], health: "all" }, {
      headers: { 'Api-Key': process.env.VITE_CROP_API_KEY }
    });
